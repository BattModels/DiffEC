...
def simulation(K0=1.0,alpha=0.5,beta=0.5,sigma=10,theta_i=10, theta_v=-10, deltaTheta = 1e-1, z_A=0.0,z_B=-1.0,z_M=1.0,z_N=-1.0,C_sup=100,d_A=1.0,d_B=1.0,d_M=1.0,d_N=1.0,Re=10000,cycles=1,number_of_interations = 5,saving_directory='./Data',save_voltammogram=False):
    """
    K0: Standard Elecrtochemical Rate Constants 
    alpha: The cathodic transfer coefficient 
    beta: The anodic transfer coefficient
    ...
    aA,bA,cA,dA, aB,bB,cB,dB, aM,bM,cM,dM, aN,bN,cN,dN, aPhi,bPhi,cPhi,dPhi = ini_coeff(n=n,XGrid=XGrid,deltaX=deltaX,deltaT=deltaT,maxX=maxX,K0=K0,alpha=alpha,beta=beta,Re=Re,d_A=d_A,d_B=d_B,d_M=d_M,d_N=d_N,z_A=z_A,z_B=z_B,z_M=z_M,z_N=z_N)
    ...
    for index in tqdm.tqdm(range(0,len(E))):
        ...
        for ii in range(number_of_interations):
            time1 = time.time()
            fx = calc_fx(K0=K0,alpha=alpha,beta=beta,x=conc,d=d,deltaX=deltaX,deltaT=deltaT,Theta=Theta,n=n,Re=Re,d_A=d_A,d_B=d_B,d_M=d_M,d_N=d_N,z_A=z_A,z_B=z_B,z_M=z_M,z_N=z_N,aA=aA,bA=bA,cA=cA,dA=dA,aB=aB,bB=bB,cB=cB,dB=dB,aM=aM,bM=bM,cM=cM,dM=dM,aN=aN,bN=bN,cN=cN,dN=dN,aPhi=aPhi,bPhi=bPhi,cPhi=cPhi,dPhi=dPhi,fx=fx)

            J = calc_jacob(K0=K0,alpha=alpha,beta=beta,x=conc,d=d,deltaX=deltaX,deltaT=deltaT,Theta=Theta,n=n,Re=Re,d_A=d_A,d_B=d_B,d_M=d_M,d_N=d_N,z_A=z_A,z_B=z_B,z_M=z_M,z_N=z_N,aA=aA,bA=bA,cA=cA,dA=dA,aB=aB,bB=bB,cB=cB,dB=dB,aM=aM,bM=bM,cM=cM,dM=dM,aN=aN,bN=bN,cN=cN,dN=dN,aPhi=aPhi,bPhi=bPhi,cPhi=cPhi,dPhi=dPhi,J=J)
    ...
    if save_voltammogram:
        ...
        df.to_csv(f'{saving_directory}/sigma={sigma:.2E} logK0={np.log10(K0):.2E} alpha={alpha:.2E} C_sup={C_sup:.0f}.csv',index=False)
...
if __name__ == "__main__":
    ...
    for alpha in [0.45]:
        for K0 in [10**-2,10**-1,10**0,10**0.5]:
            for C_sup in [5,10,20,50,100]:
                simulation(sigma=10,K0=K0,alpha=alpha,C_sup=C_sup,save_voltammogram=True)