...
def simulation(K0=1.0,kinetics="BV",alpha=0.5,beta=0.5,reorg_e=0.5,sigma=10,theta_i=10, theta_v=-10, deltaTheta = 1e-1, z_A=0.0,z_B=-1.0,z_M=1.0,z_N=-1.0,C_sup=100,d_A=1.0,d_B=1.0,d_M=1.0,d_N=1.0,Re=10000,cycles=1,number_of_interations = 5,saving_directory='./Data',save_voltammogram=False):
    """
    K0: Standard Elecrtochemical Rate Constants 
    kinetics = "BV" or "MHC"
    reorg_e: Dimensionless reogrnization energy for MHC kinetics 
    alpha: The cathodic transfer coefficient  for BV kinetics 
    beta: The anodic transfer coefficient for BV kinetics
    ...
    number_of_iterations: maximum number of Newton-Raphson iterations to solve the nonlinear system
    """
    ...
    if save_voltammogram:
        ...
        df.to_csv(f'{saving_directory}/logK0={np.log10(K0):.2E} kinetics = {kinetics} alpha={alpha:.2E} beta={beta:.2E} reorg={reorg_e:.2E} sigma={sigma:.2E} C_sup={C_sup:.2E}.csv',index=False)
