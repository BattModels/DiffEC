...
def MHIntegrand_Red(Theta_effective,reorg_e,hermgauss_degree=50):
    sample_points,weights = np.polynomial.hermite.hermgauss(hermgauss_degree)

    def integrand(u):
        return 2*jnp.sqrt(reorg_e)/(1.0+jnp.exp(-(reorg_e*(u*2.0/jnp.sqrt(reorg_e)-1.0)-Theta_effective)))
...
def MHIntegrand_Ox(Theta_effective,reorg_e,hermgauss_degree=50):
    sample_points,weights = np.polynomial.hermite.hermgauss(hermgauss_degree)

    def integrand(u):
        return -2.0*jnp.sqrt(reorg_e)/(1.0+jnp.exp(-reorg_e*(u*2.0/jnp.sqrt(reorg_e)-1.0)-Theta_effective))
...
def calMH_ox(Phi0,Theta,K0,reorg_e):
    Theta_effective = Theta - Phi0
    """
    Calculate rate constants using asymmtric MH theory
    K0: Dimensionless electrochemical rate constant 
    theta: Dimensionless overpotential 
    reorg_e: Dimensionless reorg energy 
    """

    IOx = MHIntegrand_Ox(Theta_effective=Theta_effective,reorg_e=reorg_e)
    IOx0 = MHIntegrand_Ox(Theta_effective=0,reorg_e=reorg_e) 
...
def calMH_red(Phi0,Theta,K0,reorg_e):
    Theta_effective = Theta - Phi0
    """
    Calculate rate constants using asymmtric MH theory
    K0: Dimensionless electrochemical rate constant 
    ...
    reorg_e: Dimensionless reorg energy 
    """
    IRed = MHIntegrand_Red(Theta_effective=Theta_effective,reorg_e=reorg_e)
    IRed0 = MHIntegrand_Red(Theta_effective=0,reorg_e=reorg_e)
...
#@timer
@partial(jax.jit,static_argnames=['kinetics','n'])
def calc_fx(K0,kinetics,alpha,beta,reorg_e,x,d,deltaX,deltaT,Theta,n,Re,d_A,d_B,d_M,d_N,z_A,z_B,z_M,z_N,aA,bA,cA,dA, aB,bB,cB,dB, aM,bM,cM,dM, aN,bN,cN,dN, aPhi,bPhi,cPhi,dPhi,fx,):
    h = deltaX
    ...
    if kinetics == "BV":
        ...
        Kred = calMH_red(x[4],Theta,K0,reorg_e)
        Kox = calMH_ox(x[4],Theta,K0,reorg_e)
        arow.append(0)
        aval.append((1+Kred*h/d_A)*x[0] - Kox*h/d_A * x[1] - x[5])
        arow.append(1)
        aval.append((1+Kox*h/d_B)*x[1] - Kred*h/d_B * x[0] - x[6])
        arow.append(2)
    else:
        raise ValueError
...
#@timer
@partial(jax.jit,static_argnames=["kinetics","n"])
def calc_jacob(K0,kinetics,alpha,beta,reorg_e,x,d,deltaX,deltaT,Theta,n,Re,d_A,d_B,d_M,d_N,z_A,z_B,z_M,z_N,aA,bA,cA,dA, aB,bB,cB,dB, aM,bM,cM,dM, aN,bN,cN,dN, aPhi,bPhi,cPhi,dPhi,J):
    h = deltaX
    arow = []
    acol = []
    aval = []
    if kinetics == "BV":
        ...
        Kred,Kred_Phi0 = jax.value_and_grad(calMH_red)(x[4],Theta,K0,reorg_e)
        Kox,Kox_Phi0 = jax.value_and_grad(calMH_ox)(x[4],Theta,K0,reorg_e)

        arow.append(0)
        acol.append(0)
        aval.append(Kred*h/d_A + 1.0)

        arow.append(0)
        acol.append(1)
        aval.append(-Kox*h/d_A)

        arow.append(0)
        acol.append(4)
        aval.append(h/d_A*Kred_Phi0*x[0] + h/d_A*Kox_Phi0*x[1])
        ...
        arow.append(1)
        acol.append(0)
        aval.append(-Kred*h/d_B)

        arow.append(1)
        acol.append(1)
        aval.append(Kox*h/d_B + 1.0)

        arow.append(1)
        acol.append(4)
        aval.append(-h/d_B*Kred_Phi0*x[0] - h/d_B*Kox_Phi0*x[1])
...
def ini_coeff(n,XGrid,deltaX,deltaT,maxX,K0,reorg_e,Re,d_A,d_B,d_M,d_N,z_A,z_B,z_M,z_N):
    aA = jnp.zeros(n)
    bA = jnp.zeros(n)
    cA = jnp.zeros(n)
    dA = jnp.zeros(n)
