...
def simulation(dimKf,dimKeq,cTstar,alpha=1.0,k0=5e-3,Theta=-25.667,saving_directory="./Data",save_voltammogram=False):
    """
    dimKf: The forward reaction rate constant, s^-1
    dimKeq = dimKf/dimkb, the equilibrium constant, the unit is M
    cTstar: The initial concentration of acetic acid put into solution
    ...
    jax.debug.print('Bulk concentration is {cTstar}',cTstar=cTstar)
    jax.debug.print('Steady Stae current is {current}',current=currents[-1])
    jax.debug.print('Steady Stae Flux is {flux}\n',flux=fluxes[-1])
    
    if save_voltammogram:
    
        df = pd.DataFrame({'Time':TGrid[:nt-1],'Current':currents,'Flux':fluxes})
        df.to_csv(f'keq={dimKeq:.2E} kf={dimKf:.2E},cTsatr={cTstar:.2E}.csv',index=False)
...
if __name__ == "__main__":
    ...
    simulation(dimKeq=1.7538805018417602e-05,dimKf=3467368.50452531,k0=1e-5,alpha=1.0,cTstar=0.01,save_voltammogram=True)
    simulation(dimKeq=1.7538805018417602e-05,dimKf=3467368.50452531,k0=1e-5,alpha=1.0,cTstar=0.04,save_voltammogram=True)
    simulation(dimKeq=1.7538805018417602e-05,dimKf=3467368.50452531,k0=1e-5,alpha=1.0,cTstar=0.1,save_voltammogram=True)
