...
import pickle
import pandas as pd
from SimulationHyperParameters import cycles,deltaX,deltaTheta,expanding_grid_factor,SimulationSpaceMultiple,Theta_i,Theta_v,scan_rates
...
def compute_loss(params,sigmas,experimental_fluxes):
    ...
    for i,sigma in enumerate(sigmas):
        theta_simulated, flux_simulated = simulation(sigma=sigma,K0=K0_guess,alpha=alpha_guess,beta=beta_guess,theta_i=Theta_i,theta_v=Theta_v,dA=dA_guess,dB=dA_guess)
...
if __name__ == "__main__":
    ...
    sigmas = diffECDict['sigmas']
    exp_dimensionless_files = diffECDict['exp_dimensionless_files']
    logK0_initial = diffECDict['logK0_initial']
    alpha_initial = diffECDict['alpha_initial']
    beta_initial = diffECDict['beta_initial']
