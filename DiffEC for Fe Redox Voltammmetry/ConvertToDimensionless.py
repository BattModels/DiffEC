...
colors = viridis(np.linspace(0,1,len(scan_rates)))[::-1]


for index, (nu,sheet_name) in enumerate(zip(scan_rates,sheet_names)):
    df_exp = pd.read_excel(Excel_File_Name,sheet_name=sheet_name)
    Emid = findMidPointPotential(df_exp)
    #ax.plot(df_exp.iloc[:,0],df_exp.iloc[:,1],lw=3,label=f'{nu:.3f}V/s,Emid={Emid:.3f}V')
    ax.plot(df_exp.iloc[:,0],df_exp.iloc[:,1],lw=3,label=f'{nu:.2f}V/s',color=colors[index])
...
fig.savefig('FeRedoxExperiment.png',dpi=250,bbox_inches='tight')
